...
Совпадение проверяется по границе домена: `instagram.com` исключает `instagram.com` и
`api.instagram.com`, но **не** затрагивает `cdninstagram.com`.

+ Учтите, что родительский домен исключает все дочерние: `google.com` уберёт редиректы и для
  `gemini.google.com`, и для `aistudio.google.com`.
...
+ Домен, для которого настроен редирект, не попадёт в блоклист: заблокированный домен не доходит
  до стадии редиректа, поэтому блокировка молча ломала бы его перенаправление
+ Если заблокирован **родительский** домен (например, `google.com` указан в `BLOCK`, а редирект
  настроен для `gemini.google.com`), победит блокировка и редирект работать не будет. Такие
  конфликты выводятся в лог, но блокировка сохраняется — уберите её из источника, иначе редирект
  останется нерабочим
